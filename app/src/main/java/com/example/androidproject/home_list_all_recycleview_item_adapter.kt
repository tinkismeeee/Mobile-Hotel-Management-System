...
class home_list_all_recycleview_item_adapter(private val list_all_villas_and_hotels: List<villas_and_hotels_list_item>) :
    ...
    override fun getItemCount(): Int {
        return list_all_villas_and_hotels.size
    }
    ...
    override fun onBindViewHolder(holder: list_all_villas_and_hotels_ViewHolder, position: Int) {
        ...
        if (currentItem.photoReference != null) {
            ...
            val photoUrl = "https://maps.googleapis.com/maps/api/place/photo" +
                    "?maxwidth=400" +
                    "&photoreference=${currentItem.photoReference}" +
                    "&key=$apiKey"

            Glide.with(holder.itemView.context)
                .load(photoUrl)
                .centerCrop()
                .placeholder(R.drawable.hotel_img)
                .error(R.drawable.hotel_img)
        ...
        holder.itemView.setOnClickListener {
            ...
            intent.putExtra("HOTEL_LOCATION", currentItem.location)
            intent.putExtra("HOTEL_RATING", currentItem.rating.toString())
            intent.putExtra("HOTEL_PHOTO_REFERENCE", currentItem.photoReference)
